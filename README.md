...
| `--environment-layout` (boolean) | Generates a multi-environment Terraform scaffold instead of a flat output. The scaffold includes a root module, a `modules/property` module, and per-environment `terraform.tfvars` files under `environments/dev` and `environments/prod`. <br /><br /> <blockquote><b>Notes:</b><ul><li>Must be used together with <code>--rules-as-hcl</code>, <code>--split-depth</code>, and <code>--rule-format</code>.</li><li>Requires rule format ≥ <code>v2026-01-09</code>.</li></ul></blockquote> | `akamai terraform export-property --environment-layout --rules-as-hcl --split-depth=1 --rule-format "v2026-02-16" "my-property-name"` |
